...
import { bulwarkSignerAvailable } from "./lib/bulwark/report.js";
...
const PUBLIC_BASE_URL = (process.env.PUBLIC_BASE_URL || "https://mcp.evidiq.dev/bulwark").replace(/\/$/, "");

const bypassed = isX402Bypassed();
if (bypassed) {
  console.warn("[bulwark] X402 GATE BYPASSED — Phase 1 test build. Paid tools answer without payment.");
}
...
function isMcpPath(urlPath: string): boolean {
  return urlPath === "/mcp" || urlPath === "/mcp/" || urlPath === "/bulwark/mcp" || urlPath === "/bulwark/mcp/";
}
...
const httpServer = http.createServer(async (req, res) => {
  ...
  // ── /health ──────────────────────────────────────────────────────────────
  if (urlPath === "/health" || urlPath === "/" || urlPath === "/bulwark/health") {
    res.statusCode = 200;
    ...
    res.end(
      JSON.stringify({
        ...
        signerAvailable: bulwarkSignerAvailable(),
  ...
  // ── /x402 ────────────────────────────────────────────────────────────────
  if (urlPath === "/x402" || urlPath === "/bulwark/x402") {
    res.statusCode = 200;
    res.setHeader("Content-Type", "application/json");
  ...
  // ── /skill.md ────────────────────────────────────────────────────────────
  if (urlPath === "/skill.md" || urlPath === "/bulwark/skill.md") {
...
httpServer.listen(PORT, HOST, () => {
  console.log(`[evidiq-bulwark-mcp] listening at http://${HOST}:${PORT}`);
